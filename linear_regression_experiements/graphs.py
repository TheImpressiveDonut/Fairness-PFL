...
def plot_corr(x, y, x_label, y_label, title, save_path=None, hue_control=None, hue_label=None, y_0=False):
    ...
    if hue_control is not None:
        ...
        custom_palette = ["#2E86C1"]  # Avoid the lightest and brightest colors
    ...
    plt.plot(df['x'], m * df['x'] + b, color='#E74C3C', linewidth=2, label='Trend Line')

    if y_0:
        plt.axhline(y=0, color='gray', linestyle='--', linewidth=1)

    fontsize = 12
    plt.xlabel(x_label, fontsize=fontsize, color='#2C3E50')
    plt.ylabel(y_label, fontsize=fontsize, color='#2C3E50')
    ...
    if hue_control is not None:
        ...
        cbar.set_label(hue_label, fontsize=fontsize, color='#2C3E50')
    else:
        plt.legend(loc="best", fontsize=fontsize)
    plt.grid(False)
    plt.tight_layout()
...
def plot_evolution(data, limit, save_path=None):
    data = np.vstack(data)
    clients = [0, 4, 6, 13]
    data = data[:, clients]
    # Number of i values
    ...
    # Create a range for i values
    i_values = range(num_i)

    # Create subplots
    fig, axs = plt.subplots(num_n, 1, figsize=(7, 7), sharex=True)

    for idx in range(num_n):
        axs[idx].plot(i_values, data[:, idx], color='#2E86C1',  linestyle='-', label="Aggregating MSE")
        axs[idx].axhline(y=limit[idx], color='#E74C3C', linestyle='--', label=f'Local OLS MSE, {limit[idx]:.5f}')
        axs[idx].set_ylabel('MSE')
        axs[idx].set_title(f'Hospital n°{clients[idx] + 1}')
        axs[idx].legend(loc="best")
        axs[idx].xaxis.set_major_locator(MaxNLocator(integer=True))

    plt.xlabel('Aggregating iterations')
    plt.tight_layout()
    if save_path is not None:
        plt.savefig(save_path, bbox_inches='tight')
    plt.show()